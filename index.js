...
function cleanupCaches() {
  ...
  for (const [key, value] of conversations.entries()) {
    if (now - value.updatedAt > HISTORY_TTL_MS) conversations.delete(key);
  }
  for (const [key, createdAt] of processedMessageIds.entries()) {
    if (now - createdAt > DEDUPE_TTL_MS) processedMessageIds.delete(key);
  }
}
...
function saveTurn(userId, userText, assistantText) {
  ...
  conversations.set(userId, { items, updatedAt: Date.now() });
}
...
function extractTextMessage(body) {
  ...
  if (message.type === "text") {
    return { from, id, text: message.text?.body?.trim() || "" };
  }
...
async function processIncoming(body) {
  ...
  if (!incoming?.from || !incoming?.id || !incoming.text) return;

  if (processedMessageIds.has(incoming.id)) return;
  processedMessageIds.set(incoming.id, Date.now());

  try {
    const reply = await askGemini(incoming.from, incoming.text);
    await sendWhatsAppText(incoming.from, reply);
    saveTurn(incoming.from, incoming.text, reply);
  } catch (error) {
    console.error("Message processing failed:", error);
    try {
      await sendWhatsAppText(
        incoming.from,
        "نعتذر، تعذر إتمام الطلب مؤقتًا. تم تحويل استفساركم إلى فريق خدمة العملاء."
      );
    } catch (sendError) {
      console.error("Fallback message failed:", sendError);
    }
  }
}
